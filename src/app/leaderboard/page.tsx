      ...
      <div className="flex-1 p-6 md:p-12 max-w-6xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          ...
          <div className="flex p-1 bg-surface-container rounded-xl border border-outline-variant/15">
            <button className="px-6 py-2 rounded-lg bg-surface-variant text-primary font-semibold shadow-inner transition-all">Weekly</button>
            <button className="px-6 py-2 rounded-lg text-on-surface/60 hover:text-on-surface transition-all">All-Time</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 items-end">
          {topThree.map((p) => (
            <div
              key={p.rank}
              className={`${p.order} glass-card p-${p.rank === 1 ? "8" : "6"} rounded-${p.rank === 1 ? "3xl" : "2xl"} physics-transition hover:translate-y-[-4px] ${p.rank === 1 ? "border-primary/30 ring-4 ring-primary/5 relative luminous-glow" : "group"}`}
            >
              {p.rank === 1 && (
        ...
        <div className="space-y-3">
          ...
          {others.map((user) => (
            user.isMe ? (
              <div key={user.rank} className="grid grid-cols-12 items-center px-8 py-6 bg-surface-variant/60 rounded-2xl border-l-4 border-primary shadow-xl shadow-primary/5 relative overflow-hidden">
                ...
                <div className="col-span-3 text-right font-mono text-primary font-bold">{user.xp}</div>
                <div className="col-span-3 text-right font-bold text-secondary-fixed">{user.ret}</div>
              </div>
            ) : (
              <div key={user.rank} className="grid grid-cols-12 items-center px-8 py-5 bg-surface-container rounded-2xl hover:bg-surface-variant/40 transition-colors physics-transition group">
        ...
        <div className="mt-12 text-center">
          <button className="bg-surface-container-highest px-8 py-3 rounded-full text-primary font-bold border border-primary/20 hover:bg-primary hover:text-on-primary transition-all physics-transition">
            View Full Rankings
          </button>
        </div>
      </div>
